...
def parse_config_from_text(config_text):
    ...
    while i < len(lines):
        ...
        if ":" not in line:
            raise ValueError(f"Invalid config block start: '{line}'")
        ...
        try:
            ...
            raise ValueError(f"Invalid config block start: '{line}'") from e
        ...
        while i < len(lines) and (lines[i].startswith(" ") or not lines[i]):
            ...
            if expr_type == "math":
                if key == "int":
                    ...
                    assert (
                        1 < len(tokens) <= 3
                    ), "float must have between 2 and 3 arguments"
                    tokens[1:] = tokens[1:][::-1]
                    kwargs = _parse_tokens_to_kwargs(
                    ...
                    raise ValueError(f"Unknown math sub-key: '{key}'")
                ...
                if key == "start":
                    ...
                    raise ValueError(f"Unknown date sub-key: '{key}'")
